...
describe('Login', () => {
    ...
    it('deve cadastrar um aluno quando ele informa dados válidos', async () => {
        ...
        expect(cadastroAlunoResposta.body.nome).to.equal("Hanrrison Oliveira");
        expect(cadastroAlunoResposta.body.email).to.equal("hanrrison.oliveira@example.com");
    ...
    it.only('deve negar o cadastro de um aluno quando ele já existe', async () => {
        ...
        expect(cadastroAlunoResposta.body.error).to.equal('error', 'Já existe um aluno cadastrado com essa matrícula ou e-mail.');
    })
})